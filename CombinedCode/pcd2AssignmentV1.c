...
#define IC_SIZE 13
#define USERNAME_SIZE 30#pragma warning (disable:4996)
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <time.h>
	...
	while (backToMain == false) {
		...
		case 1:
			...
			case 1:
				...
				For example, we want to modify our email. IC is the first datafield of the struct, followed by username, then password, then email. To move the pointer back to the very beginning
				of the username field, we tell fseek how far back we want to go, which is calculated by the sum of the size of datafields before email, minus the total size of the struct.

																		(IC_SIZE + USERNAME_SIZE + PASSWORD_SIZE) - sizeof(Member)

				...and by plugging it into fseek, we effectively moved to the front of the 'email' datafield, allowing us to simply fwrite the new data.
				*/
